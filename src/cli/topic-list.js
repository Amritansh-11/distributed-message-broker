...
export function listTopicsCli(options = {}) {
  ...
  return new Promise((resolve) => {
    ...
    socket.on('connect', () => {
      console.log('[Topic CLI] Sending LIST_TOPICS request...');
      const reqObj = ProtocolRequest.listTopics();
    ...
    socket.on('data', (chunk) => {
      ...
      for (const frame of frames) {
        if (!frame.error) {
          ...
          if (!decoded.error) {
            console.log('[Topic CLI] Active Topics:', JSON.stringify(decoded.parsed, null, 2));
          }
        }
      }
      socket.end();
